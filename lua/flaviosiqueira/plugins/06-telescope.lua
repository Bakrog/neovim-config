...
return {
    ...
    config = function()
        ...
        vim.keymap.set("n", "<leader>fs", function() builtin.find_files({ hidden = true, no_ignore = true }) end,
            { desc = "Find Files (Sys)" })                                                -- Original keymap slightly modified
        vim.keymap.set("n", "<leader>ps", function() builtin.grep_string({ search = vim.fn.input("Grep For > ") }) end,
            { desc = "Grep String" })                                                     -- Original keymap kept
